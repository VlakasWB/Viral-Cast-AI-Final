<script lang="ts">
	...
	let name = $state(data.ingredientCatalog.name);
	let description = $state(data.ingredientCatalog.description || '');
	let baseUomUuid = $state(data.ingredientCatalog.base_uom.uuid);
let minimumStock = $state(data.ingredientCatalog.minimum_stock.toString());
let shelfLifeDays = $state(data.ingredientCatalog.shelf_life_days.toString());
...
<div class="container mx-auto p-4">
	<div class="bg-white rounded-lg shadow-md p-6">
		<h1 class="text-2xl font-bold mb-6">Edit Katalog Bahan</h1>
