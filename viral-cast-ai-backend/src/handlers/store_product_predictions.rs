...
use crate::dto::store_product_predictions::{
    GenerateStorePredictionParams, ProductPredictionDto, StorePredictionResponseDto,
    WeatherBriefDto,
};
...
use crate::repository::store_product_predictions::{NewStoreProductPrediction, RegionContext};
...
struct LlmResponsePayload {
    ...
    products: Vec<LlmProductPrediction>,
}
...
#[derive(Debug, Deserialize)]
struct LlmProductPrediction {
    product_uuid: Uuid,
    demand_label: String,
    demand_probability: Option<f32>,
    recommended_stock_qty: Option<f64>,
...
pub async fn generate_store_product_predictions_handler(
    State(state): State<Arc<AppState>>,
    Extension(jwt_auth): Extension<JWTAuthMiddleware>,
    Query(params): Query<GenerateStorePredictionParams>,
    ...
    if products.is_empty() {
        return Err((
            ...
            Json(json_error(
                400,
                "No active products".to_string(),
                "Belum ada produk aktif sehingga prediksi tidak dapat dibuat.".to_string(),
            )),
        ));
    }
    ...
    let context_json = json!({
        "store": {
            ...
            "name": store.name,
            "location": {
                "province": region_context.provinsi,
                "regency": region_context.kotkab,
                "district": region_context.kecamatan,
                "village": region_context.desa,
    ...
    let (system_prompt, user_prompt) = build_prompts(target_date, &context_json);
    // Logging prompt dan parameter LLM untuk diagnosa
    let base_model =
    ...
    info!(
        ...
        "Preparing LLM request with prompts"
    );
    info!("LLM system_prompt: {}", system_prompt);
    info!("LLM user_prompt: {}", user_prompt);
    info!("LLM context_json: {}", context_json.to_string());

    // Simpan prompt ke file agar bisa ditinjau
    let log_dir =
    ...
    let file_name = format!(
        "store_predictions_{}_{}_{}.json",
        store.uuid, region_context.region_code, ts_ms
    );
    let mut path = PathBuf::from(&log_dir);
    path.push(file_name);
    let prompt_log_json = json!({
        "timestamp_ms": ts_ms,
        "endpoint": "/api/v1/stores/predictions",
    ...
    match serde_json::to_string_pretty(&prompt_log_json) {
        Ok(content) => {
            if let Err(e) = tokio::fs::write(&path, content).await {
                ...
                info!("LLM prompt log written to '{}'", path.display());
            }
        }
        Err(e) => warn!("Failed to serialize LLM prompt log JSON: {}", e),
    }
    // Gunakan model generatif untuk prediksi (bisa dikonfigurasi melalui GROQ_PREDICTIONS_MODEL)
    let predictions_model = resolve_predictions_model();
    info!("Using predictions model: {}", predictions_model);
    ...
    .map_err(|e| {
        (
            ...
            Json(json_error(
                ...
                format!("Gagal meminta prediksi dari model LLM: {e}"),
            )),
        )
    })?;
    ...
    if was_repaired {
        warn!("LLM response required repair before parsing into JSON");
    }

    let product_lookup: HashMap<Uuid, &ProductSnapshot> =
        products.iter().map(|p| (p.uuid, p)).collect();

    let mut new_prediction_rows: Vec<NewStoreProductPrediction> = Vec::new();
    for product_prediction in payload.products.iter() {
        if !product_lookup.contains_key(&product_prediction.product_uuid) {
            continue;
        }

        let demand_label = product_prediction.demand_label.trim().to_uppercase();
        let demand_probability = product_prediction
            .demand_probability
            .map(|v| v.clamp(0.0, 1.0));
        let recommended_qty = product_prediction
            .recommended_stock_qty
            .unwrap_or(0.0)
            .max(0.0);
        let recommended_stock_qty =
            Decimal::from_f64(recommended_qty).unwrap_or_else(|| Decimal::from(0));
        let forecast_error_pct = product_prediction
            .forecast_error_margin_pct
            .or_else(|| fallback_error_margin(demand_probability));

        let base_reasoning = product_prediction
            .reasoning
            .as_ref()
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
        ...
        new_prediction_rows.push(NewStoreProductPrediction {
            store_uuid: store.uuid,
            product_uuid: product_prediction.product_uuid,
    ...
    if new_prediction_rows.is_empty() {
        return Err((
            ...
            Json(json_error(
                ...
                "Model LLM tidak menghasilkan prediksi apapun untuk produk yang diberikan."
                    .to_string(),
            )),
        ));
    }

    store_predictions_repository::upsert_predictions(&state.db, &new_prediction_rows)
        .await
        .map_err(internal_error)?;

    let persisted = store_predictions_repository::get_predictions_for_store(&state.db, store.uuid)
        .await
        .map_err(internal_error)?;
    ...
    Ok(Json(ApiResponse {
        ...
        message: "Prediksi produk berhasil dibuat".to_string(),
        data: response_body,
        errors: serde_json::json!({}),
    }))
}

pub async fn get_store_product_predictions_handler(
    State(state): State<Arc<AppState>>,
    Extension(jwt_auth): Extension<JWTAuthMiddleware>,
) -> Result<impl IntoResponse, (StatusCode, Json<Value>)> {
    let user_uuid = jwt_auth.user.uuid;
    ...
    let predictions =
        store_predictions_repository::get_predictions_for_store(&state.db, store.uuid)
            .await
            .map_err(internal_error)?;
    ...
    if predictions.is_empty() {
        return Err((
            ...
            Json(json_error(
                ...
                "Belum ada prediksi yang tersimpan untuk store ini.".to_string(),
            )),
        ));
    }

    let response_body = build_prediction_response(
        store.uuid,
        &region_context.unwrap_or(RegionContext {
    ...
    Ok(Json(ApiResponse {
        code: 200,
        status: "success".to_string(),
        message: "Prediksi produk berhasil diambil".to_string(),
        data: response_body,
        errors: serde_json::json!({}),
    }))
}
...
fn build_prediction_response(
    ...
    persisted: &[StoreProductPredictionWithProduct],
) -> StorePredictionResponseDto {
    let weather_numbers = store_predictions_repository::aggregate_prediction_fields(persisted);
    let weather = if persisted.is_empty() {
        None
    } else {
        Some(WeatherBriefDto {
            summary: persisted
                .iter()
    ...
    let products = persisted
        .iter()
        .map(|p| ProductPredictionDto {
            product_uuid: p.product_uuid,
            product_name: p.product_name.clone(),
            product_sku: p.product_sku.clone(),
            demand_label: p.demand_label.clone(),
            demand_probability: p.demand_probability,
            recommended_stock_qty: p.recommended_stock_qty,
            llm_reasoning: p.llm_reasoning.clone(),
            forecast_error_margin_pct: extract_forecast_error_margin(p)
                .or_else(|| fallback_error_margin(p.demand_probability)),
        })
        .collect();
...
fn extract_forecast_error_margin(prediction: &StoreProductPredictionWithProduct) -> Option<f32> {
    let response = prediction.llm_response.as_ref()?;
    let products = response.get("products")?.as_array()?;
    let target_uuid = prediction.product_uuid.to_string();
...
fn summarize_weather(slots: &[WeatherSlotSnapshot]) -> Value {
    ...
    let dominant_weather = weather_counts
        ...
        .map(|(desc, _)| desc)
        .unwrap_or_else(|| "Cuaca bervariasi".to_string());
    ...
    json!({
        ...
        "precipitation_total_mm": if precipitation_sum > 0.0 { Some((precipitation_sum * 10.0).round() / 10.0) } else { None }
    })
}
...
async fn load_weather_slots(
    ...
    // Try to hydrate from existing cached forecast run (raw JSON)
    match weather_repository::get_latest_raw_forecast_run_json(&state.db, region_code).await {
            ...
            match serde_json::from_value::<BMKGWeatherResponse>(raw_json.clone()) {
                Ok(weather_response) => {
                    ...
                    .await
                    {
                        warn!(
                            region = region_code,
                            error = %e,
                            "[store_predictions] failed to upsert forecasts from cached run"
                        );
                    } else {
                ...
                Err(e) => {
                    warn!(
                        region = region_code,
                        error = %e,
                        "[store_predictions] failed to decode cached bmkg_forecast_run raw JSON"
                    );
                }
            }
        }
        ...
        Err(e) => {
            warn!(
                region = region_code,
                error = %e,
                "[store_predictions] failed to read cached bmkg_forecast_run"
            );
        }
    }
    ...
    if state.bmkg_save_json {
        if let Err(e) = dump_weather_json(region_code, &weather) {
            warn!(
                region = region_code,
                error = %e,
                "[store_predictions] failed to write weather JSON dump"
            );
        }
    }
...
fn build_prompts(target_date: NaiveDate, context_json: &Value) -> (String, String) {
    let system_prompt = "Anda adalah analis retail untuk bisnis F&B Indonesia. \
        Tugas Anda adalah memprediksi permintaan produk untuk besok berdasarkan daftar produk, harga, \
        dan prakiraan cuaca BMKG. Sampaikan rekomendasi stok yang realistis dengan narasi kreatif yang \
        menyinggung suhu minimum-maksimum, kelembapan, dan kondisi hujan/cerah. Jelaskan bagaimana \
        permintaan dapat berubah jika cuaca membaik (lebih cerah/kering) atau memburuk (lebih hujan/panas). \
        Sertakan estimasi margin kesalahan (%) agar manajer toko dapat menilai risiko.".to_string();

    let user_prompt = format!(
        "Gunakan konteks JSON berikut untuk memprediksi penjualan produk pada tanggal {date}. \
        Kembalikan output dalam format JSON tanpa teks tambahan, mengikuti struktur persis ini: \
        {{
          \"summary\": string,
          \"weather_brief\": {{
            \"summary\": string,
          ...
          \"products\": [
            {{
              \"product_uuid\": \"UUID\",
              \"demand_label\": \"HIGH\"|\"MEDIUM\"|\"LOW\"|\"STABLE\",
              \"demand_probability\": number|null,
        ...
        Instruksi tambahan: \
        1. Gunakan bahasa Indonesia profesional dan ringkas. \
        2. Di setiap reasoning, sebutkan kisaran suhu (misal \"24-30°C\"), kondisi cuaca (cerah/hujan/lembap), \
           dan jelaskan bagaimana perubahan cuaca yang lebih panas/dingin atau lebih kering/basah bisa menaikkan \
           atau menurunkan permintaan. \
        3. Isi \"forecast_error_margin_pct\" sebagai margin kesalahan absolut (0-100) yang masuk akal \
           (biasanya 5-35) agar pengguna memahami ketidakpastian. \
        Konteks:\n{context}",
        date = target_date,
...
async fn call_groq(
    ...
    if let Ok(payload_json) = serde_json::to_string(&req_body) {
        ...
        warn!("Failed to serialize Groq request payload for logging");
    }
    ...
    for attempt in 1..=GROQ_MAX_ATTEMPTS {
        ...
        if status == ReqwestStatusCode::TOO_MANY_REQUESTS {
            ...
            if attempt == GROQ_MAX_ATTEMPTS {
                return Err(format!(
                    "Groq rate limit reached after {attempt} attempts. Last body: {body}"
                ));
            }
        ...
        return serde_json::from_str::<GroqApiResponse>(&body).map_err(|e| {
            let snippet: String = body.chars().take(512).collect();
...
// Versi yang memungkinkan memilih model secara eksplisit
...
fn coerce_payload_from_value(value: &Value) -> Option<LlmResponsePayload> {
    ...
    let mut products: Vec<LlmProductPrediction> = Vec::new();
    if let Some(arr) = obj.get("products").and_then(|v| v.as_array()) {
        for item in arr {
            if let Some(product) = coerce_product_prediction(item) {
...
fn coerce_product_prediction(value: &Value) -> Option<LlmProductPrediction> {
    let obj = value.as_object()?;
    let uuid_value = obj.get("product_uuid")?;
    let uuid_str = match uuid_value {
        Value::String(s) => s.trim(),
        _ => return None,
    };
    let product_uuid = Uuid::parse_str(uuid_str).ok()?;

    let demand_label = obj
        .get("demand_label")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_else(|| "MEDIUM".to_string());

    let demand_probability = obj.get("demand_probability").and_then(|v| value_to_f32(v));

    let recommended_stock_qty = obj
        .get("recommended_stock_qty")
