...
use crate::{dto::auth_token, models::user::User, AppState};
...
pub async fn auth(
    ...
    let access_token_details = match auth_token::verify_jwt_token(
    ...
    let redis_token_user_uuid = if let Some(v) = redis_token_user_uuid_opt {
        ...
        if let Some(v) = store.get(&access_token_uuid.to_string()).cloned() {
            v
        } else {
            let error_response = ErrorResponse {
                code: 401,
    ...
    let user_uuid = uuid::Uuid::parse_str(&redis_token_user_uuid).map_err(|_| {
        let error_response = ErrorResponse {
            code: 401,
            status: "UNAUTHORIZED",
            message: "Token is invalid or session has expired".to_string(),
